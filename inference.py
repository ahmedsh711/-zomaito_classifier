...
def preprocess_input(input_data, feature_names):
    """
    Convert input dictionary into a DataFrame matching training features.
...
def predict(model, input_data, feature_names):
    ...
    Returns:
        dict with 'prediction', 'success_probability', 'confidence'
    """
    X = preprocess_input(input_data, feature_names)
    
    prediction = model.predict(X)[0]
    probability = model.predict_proba(X)[0]  # float for single-row input
